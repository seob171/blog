...
function ResponsiveModal({ trigger, title, description, open, setOpen, children, preventInteractOutside }: Props) {
  const isScreenXS = useMediaQuery(`(max-width:${SCREEN_SIZE.sm - 1}px)`);

  if (isScreenXS) {
    return (
      <CustomDrawer open={open} onOpenChange={setOpen} trigger={trigger} title={title} description={description}>
        {children}
      </CustomDrawer>
    );
  }
