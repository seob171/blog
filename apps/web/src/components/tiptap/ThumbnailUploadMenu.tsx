...
function ThumbnailUploadMenu() {
  ...
  return (
    <ResponsiveModal
      ...
      setOpen={setOpen}
      trigger={
        <Button variant="outline" className="w-fit h-auto p-0 my-4 rounded-lg overflow-hidden">
          <div className="relative w-44 aspect-video">
            <Image src={thumbnailImage} alt="uploaded thumbnail" className={`${thumbnailImage ? '' : 'hidden'}`} />
            <div className={`${thumbnailImage ? 'hidden' : 'flex gap-x-2 items-center justify-center w-full h-full'}`}>
              <span>썸네일 업로드</span>
