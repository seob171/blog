...
>(
  options?: Omit<UseMutationOptions<TData, TError, TVariable, TContext>, 'mutationFn'>
  ...
  return useMutation<TData, TError, TVariable, TContext>({
    ...
    onMutate: async (variables): Promise<TContext> => {
      ...
      const previousPostList = queryClient.getQueryData<Array<PrismaModels['posts']>>(POST_QUERY_KEY.itemList()) ?? [];

      queryClient.setQueryData<Array<PrismaModels['posts']>>(POST_QUERY_KEY.itemList(), (postList) => {
    ...
    onError: (err, variables, context) => {
      queryClient.setQueryData(POST_QUERY_KEY.itemList(), context?.previousPostList ?? []);
      toast({
        title: '포스트 삭제 실패',
